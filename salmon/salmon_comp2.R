...
stan_file = "salmon/salmon_new.stan"
...
ggplot(farm_data, aes(x = time, y = harvested)) +
  geom_line() +
  geom_line(aes(x = time, y = biomass), color = "blue") +
  #geom_point() +
...
ggplot(farm_data[40:53,], aes(x = time, y = mean_weight_har_salmon)) +
  geom_line() +
  geom_ribbon(aes(ymin = mean_weight_har_salmon  - 1.96*sd_weight_har_salmon, 
              ...
              fill = "grey70", alpha = 0.4) + 
  #geom_point() +
  labs(title = "Harvested weight over time", x = "Days", y = "Biomass") +
  theme_minimal()
...
ggplot(farm_data, aes(x = time, y = mean_weight)) +
  ...
  geom_ribbon(aes(ymin = mean_weight   - 1.96*sd_weight , 
              ...
              fill = "grey70", alpha = 0.4) + 
  #geom_point() +
  labs(title = "Salmon weight over time", x = "Days", y = "Biomass") +
  theme_minimal()
